...
const AnimatedKeyboardAvoidingView = react_native_1.Animated.createAnimatedComponent(
...
const ModalizeBase = (
  ...
  const dragValue = react_native_1.Animated.add(
    react_native_1.Animated.multiply(dragY, componentDragEnabled ? 1 : cancelTranslateY),
    diffClamp,
  );
  const value = react_native_1.Animated.add(
    ...
    react_native_1.Animated.parallel([
      ...
      spring
        ? react_native_1.Animated.spring(translateY, {
            ...get_spring_config_1.getSpringConfig(spring),
            toValue,
            useNativeDriver: USE_NATIVE_DRIVER,
          })
  ...
  const handleGestureEvent = react_native_1.Animated.event(
  ...
  const renderComponent = (component, name) => {
    ...
    const obj = react_native_1.StyleSheet.flatten(tag?.props?.style);
